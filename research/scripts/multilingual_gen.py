...
def _load_algorithm(
    ...
    so the v1 study's temperature factor (01 §2) is reproducible. torch_dtype
    mirrors the service flag ("auto"/"fp32"/"bf16") for CPU throughput.
    """
...
def _add_common(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        help="Model dtype: auto (fp32 default), fp32, or bf16 (faster on CPU)",
    )
    p.add_argument(
        "--offline",
        action="store_true",
