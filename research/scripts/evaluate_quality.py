...
(original, candidate) texts:
  ...
  ppl             GPT-2 large mean-token perplexity of the *candidate*
                  ...
                  generator")
  bertscore       BERTScore F1, roberta-large (deberta-xlarge-mnli is
                  broken under transformers>=5: OverflowError in
                  set_truncation_and_padding; override via
                  WATERMARKS_BERTSCORE_MODEL), rescale_with_baseline=True
...
# transformers >= 5.x breaks bert_score's deberta tokenizer path
# (OverflowError in set_truncation_and_padding); roberta-large is the
# bert_score default and works. Override with WATERMARKS_BERTSCORE_MODEL.
BERTSCORE_MODEL = os.environ.get("WATERMARKS_BERTSCORE_MODEL", "roberta-large")
