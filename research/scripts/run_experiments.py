...
class RunContext:
    ...
    def __init__(self, args: argparse.Namespace, upstream: Path) -> None:
        ...
        # Quality metrics (evaluate_quality.py) need the isolated metrics env
        # (research/requirements-quality.txt): repo .venv-quality when present,
        # else an explicit --quality-python, else fall back to the MarkLLM
        # python with a warning (metric loaders degrade to None + warnings).
        self.quality_python = None
        if getattr(args, "quality_python", None):
            self.quality_python = str(args.quality_python)
        else:
            cand = Path(__file__).resolve().parents[2] / ".venv-quality" / "bin" / "python"
            if cand.is_file():
                self.quality_python = str(cand)
        if self.quality_python is None:
            print(
                "[warn] quality-metrics env not found (repo .venv-quality or --quality-python); "
                "evaluate_quality will run with the MarkLLM python and may report metric warnings",
                file=sys.stderr,
            )
            self.quality_python = self.python
        ...
        # CLI smoke controls: --prompts / --seeds limit generation to a
        # subset of the locked 25x5 matrix (budget/plan already use them;
        # the generate loop must too, so small smokes stay cheap on CPU).
        self.prompts = int(getattr(args, "prompts", PROMPTS) or PROMPTS)
        self.seeds = [
            int(x)
            for x in str(
                getattr(args, "seeds", ",".join(str(s) for s in SEEDS))
            ).split(",")
            if x.strip()
        ]
    ...
    def worker_for(
        ...
        *,
        ...
        # bf16: opt-1.3b fp32 cached decode is ~8x slower on this aarch64
        # CPU-only box; bf16 keeps same-config gen/detect consistent and makes
        # the EN core feasible (measured ~0.5 s/token vs ~4 s/token fp32).
...
def stage_generate(
    ...
    if dry_run:
        ...
        return [
            ...
            f"# {ctx.prompts * len(ctx.seeds)} watermark requests over stdin -> {out_file}",
        ]
    ...
    rows: list[dict[str, Any]] = []
    for pidx in range(1, ctx.prompts + 1):
...
def _run_one_attack(
    ...
    with tempfile.TemporaryDirectory(prefix="wm-attack-") as raw_td:
        ...
        if attack == "none":
            # Control row: the watermarked artifact unchanged (A0 baseline).
            return original, None, None, time.monotonic() - started
...
def stage_evaluate(
    ...
    if dry_run:
        return [
            f"{ctx.python} analyze_roc.py --scores {scores_file} --out {metrics_file}",
            f"{ctx.quality_python} evaluate_quality.py --input <pairs> --out {quality_file}",
        ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--quality-python",
        default=None,
        help="Python interpreter for evaluate_quality.py "
        "(default: repo .venv-quality/bin/python when present, else MarkLLM venv python)",
    )
